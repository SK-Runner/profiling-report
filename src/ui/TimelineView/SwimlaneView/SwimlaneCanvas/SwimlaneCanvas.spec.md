...
27. **PR-CANVAS-027** — Unmodified drag past 4px draws the marquee and commits intersecting events.
28. **PR-CANVAS-028** — a marquee hitting nothing commits an empty selection.
29. **PR-CANVAS-029** — marquee suppresses tooltip/select; `pointerleave` does not cancel.
30. **PR-CANVAS-030** — Escape during the marquee drag cancels without committing.
31. **PR-CANVAS-031** — drag never pans; Shift+wheel and a dominant trackpad `deltaX` pan instead, while a vertical scroll with incidental `deltaX` still scrolls lanes.
32. **PR-CANVAS-032** — in measureMode, drag measures and never marquees.
33. **PR-CANVAS-033** — live marquee emits its time extent as `multi-select-span`; end/cancel emits null.
...
## Changelog
- **2026-08-26** — Wheel pan takes the dominant axis, so a vertical trackpad scroll with incidental `deltaX` still scrolls lanes; PR-CANVAS-031.
- **2026-08-26** — Product gesture flip: unmodified drag marquees (measure mode still wins), drag no longer pans, pan moves to Shift+wheel / trackpad `deltaX`; live `multi-select-span` for axis Δt; PR-CANVAS-027/029/031/032/033.
